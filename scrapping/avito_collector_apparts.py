...
# -*- coding: utf-8 -*-
"""
Collecteur d'annonces Avito - Appartements a vendre a Casablanca
================================================================
...
Usage:
    python avito_collector.py

Le script peut etre interrompu et relance: il reprend automatiquement
grace au fichier checkpoint.

Sortie:
    avito_casablanca_appartements.xlsx
    avito_checkpoint.json (interne, peut etre supprime apres)
"""
...
BASE_URL = "https://www.avito.ma/fr/casablanca/appartements-à_vendre"
...
AGES = {0: "Moins de 1 an", 1: "1-5 ans", 2: "6-10 ans", 3: "11-20 ans", 4: "21+ ans"}
...
OUTPUT_FILE = "avito_casablanca_appartements.xlsx"
CHECKPOINT_FILE = "avito_checkpoint.json"
...
def extract_title_from_url(href):
    ...
    if "/appartements/" not in href:
        return ""
    url_path = href.split("/appartements/")[-1]
    # Enlever .htm a la fin
    if url_path.endswith(".htm"):
        url_path = url_path[:-4]
    # Enlever l'ID a la fin (_NNNN)
...
def parse_listings(html):
    ...
    for link in soup.find_all("a", href=True):
        ...
        data = {
            ...
            "titre": extract_title_from_url(href),
            "prix_dh": None,
            "mensualite_dh": None,
            "chambres": None,
            "salles_de_bain": None,
        ...
        # Secteur
        m = re.search(r"dans Casablanca,\s+([A-Za-zÀ-ÿ' \-]+?)(?=\s+(?:[A-ZÀ-Ý]|\d|Studio|Duplex|Appart))",
                      text)
        if m:
            secteur = m.group(1).strip()
            if len(secteur) < 50:
        ...
        # Regex corrigee : nombre avec separateurs d'espace type "9 450" ou "12 000"
        m = re.search(r"(?<!\d)(\d{1,3}(?:\s\d{3})+|\d{4,})\s*DH\s*/\s*mois", text)
        if m:
            try:
        ...
        # CORRECTIF : l'ancienne regex (\d[\d\s]*) capturait le chiffre de l'etage
        # (ex: "Etage 4") et le collait au prix reel => "4 1 700 000" = 41700000
        # La nouvelle regex exige un nombre bien forme avec separateurs d'espace
        # OU un nombre brut d'au moins 6 chiffres (min 100 000 DH pour un appart).
        # Le lookbehind (?<!\d) empeche de demarrer au milieu d'un nombre.
        prix_matches = re.findall(r"(?<!\d)(\d{1,3}(?:\s\d{3})+|\d{6,})\s*DH(?!\s*/)", text)
        if prix_matches:
            try:
        ...
        # Vendeur (debut du texte)
        m = re.match(
            r"^(.+?)(?=\s*il y a|\s*Premium|\s*Star|\s*Vérifié|\s*Appart)",
            text
        )
        if m:
...
def export_to_excel(annonces, filename=OUTPUT_FILE):
    ...
    headers = [
        ...
        "Secteur/Quartier", "Condition", "Standing", "Age du bien",
    ...
    widths = [12, 45, 14, 16, 10, 10, 12, 14, 22, 14, 16, 14, 22, 10, 18, 60]
...
def main():
    print("=" * 60)
    print("Collecteur Avito - Appartements a vendre a Casablanca")
    print("=" * 60)
    nb = len(CONDITIONS) * len(STANDINGS) * len(AGES)
    print("Filtres:", len(CONDITIONS), "conditions x",
          len(STANDINGS), "standings x", len(AGES), "ages =", nb, "combinaisons")
